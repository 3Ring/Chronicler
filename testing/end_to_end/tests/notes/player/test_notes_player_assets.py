...
@pytest.mark.xfail
def test_new_session_form_is_missingTODO(mock: Mock):
    with mock.test_manager(test_new_session_form_is_missingTODO):
        raise ExpectedException()


@pytest.mark.xfail
def test_edit_session_menu_is_missingTODO(mock: Mock):
    with mock.test_manager(test_edit_session_menu_is_missingTODO):
        raise ExpectedException()


@pytest.mark.xfail
def test_edit_session_form_is_missingTODO(mock: Mock):
    with mock.test_manager(test_edit_session_form_is_missingTODO):
        raise ExpectedException()
