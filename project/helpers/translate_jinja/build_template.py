...
def build_template(filename: str) -> list:
    """
    ...
    raw_html = _get(filename)
    cleaned_list = _clean(raw_html)
    new_list = _read(cleaned_list)
    return new_list


def _clean(html: str) -> list:
    """
...
def _read(html_list: list) -> list:
    ...
    for i, line in enumerate(html_list):
        ...
        if path:
            return html_list[:i] + build_template(path) + _read(html_list[i + 1 :])
    return html_list
