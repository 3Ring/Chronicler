...
function new_check_box_logic(note_id_number) {
    let checkbox_draft = document.querySelector(`input[data-id_notecheckboxprivate="${note_id_number}"]`)
    , checkbox_to_dm = document.querySelector(`input[data-id_notecheckboxToDm="${note_id_number}"]`);
...
function fill_new_note(new_note, session_number, note_id_number, note_text)    {
        ...
        // insert note body
        let element__notes_body = document.querySelector( `span[data-id_noteText="${note_id_number}"]` );
...
function insert_socket(editted_note, note_id_number, note_text)    {
    ...
    // insert note body
    var element__notes_body = document.querySelector( `span[data-id_noteText="${note_id_number}"]` );
    element__notes_body.innerHTML = note_text;
...
function fill_note_made_public(new_note, note_id_number, note_text) {
    let element__placeholder = document.querySelector(`span[data-placeholder_flag_id="${note_id_number}"]`);
...
socket.on('make_filler', function(note_id_number, ordered_session_note_list, list_location, session_number, text, is_draft, to_dm, dm_id, game_id, user_id, note_id) {
    ...
    while (!found) {
        if (list_location == 0) {
            found = document.querySelector(`ul[data-idSession='${session_number}']`);
            top_of_session_flag = true;
        } else {
            let previous_note_id = ordered_session_note_list[list_location - 1];
            let element__previous = document.querySelector(`li[data-id_notecont="${previous_note_id}"]`);
